...
export function VentasModule() {
  ...
  const handleVenta = (product: Product) => {
      setShowForm(true);
      // Aquí se podría pre-seleccionar el producto en el formulario
  ...
  const sales: Sale[] = [
    {
      id: 'V-001',
      client: 'Agropecuaria Del Valle SAC',
      document: '20123456789',
      products: [
        { name: 'Pico Agrícola', quantity: 20, price: 45 },
        { name: 'Azadón Reforzado', quantity: 15, price: 38 }
      ],
      total: 1470,
      date: '2026-05-01',
      status: 'completado'
                            ...
                            {['Tarjeta', 'Yape', 'Plim'].map((method) => (
                                ...
                                >
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                                        method === 'Yape' ? 'bg-[#742284]' : method === 'Plim' ? 'bg-[#00d1ce]' : 'bg-slate-900'
                                    }`}>
